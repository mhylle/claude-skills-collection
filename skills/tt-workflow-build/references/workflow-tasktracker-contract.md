...
See also (background memory): `reference-tasktracker-concurrency-model`, `reference-tasktracker-mcp-location`, `reference-workflow-tool-main-loop-only`, `project-concurrency-rearchitecture`.

**Update (2026-05-31) — leases shipped, but the parent-owns rule stays absolute for in-run agents.** The per-agent work-lease registry (project `577a42ad`, phases 93–97) is now LIVE and always-on. It was expected to relax this rule — but a closer reading of `mcp-server/lib/agent-id.js` shows it does NOT for parallel agents in a single Workflow run: the agentId is `mcp:<CLAUDE_CODE_SESSION_ID>`, memoized per MCP process, and all in-session workflow agents share ONE MCP process → ONE agentId → ONE active-task pointer + ONE lease. Distinct leases require distinct agentIds, which in-run agents don't have. So the leases relax cross-**session** concurrency (two separate Claude Code sessions on one project), not in-**run** concurrency. Relaxing the in-run rule would require either (a) workflow agents calling the backend REST `/leases` + write endpoints DIRECTLY with their own minted lease tokens (bypassing the shared stdio MCP), or (b) reworking the MCP to carry a stateless per-call lease token instead of process-global active-task state. Until one of those ships, **the parent funnels all tasktracker writes — absolutely.**
